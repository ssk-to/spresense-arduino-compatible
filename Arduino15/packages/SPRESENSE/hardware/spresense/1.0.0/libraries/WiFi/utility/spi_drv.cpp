...
{
    // select is performed before transfering data
}

void SpiDrv::spiSlaveDeselect()
{
    // deselect is performed after transfering data
}
...
#define waitSlaveReady()    (digitalRead(WIFI_READY) == LOW)
#define waitSlaveSign()     (digitalRead(WIFI_READY) == HIGH)
#define waitSlaveSignalH()  while(digitalRead(WIFI_READY) != HIGH) {}
#define waitSlaveSignalL()  while(digitalRead(WIFI_READY) != LOW) {}
...
int SpiDrv::waitResponseData16(uint8_t cmd, uint8_t* param, uint16_t* param_len)
{
    ...
    IF_CHECK_START_CMD(_data) {
        ...
        if (numParam != 0) {
            readParamLen16(param_len);
            for (ii = 0; ii < (*param_len); ++ii)
                param[ii] = spiTransfer(DUMMY_DATA); // Get Params data
        }
        ...
        if (_numParam != 0) {
            for (i = 0; i<_numParam; ++i) {
                ...
                for (ii = 0; ii < params[i].paramLen; ++ii)
                    params[i].param[ii] = spiTransfer(DUMMY_DATA); // Get Params data
            }
        }
        else {
